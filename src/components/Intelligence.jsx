...
function MiniBrowser() {
  return (
    <div className="mini-browser relative mx-auto mt-12 h-[220px] w-full max-w-[410px] overflow-hidden sm:mt-16 sm:h-[230px]">
      <div className="absolute left-0 top-4 h-[130px] w-[58%] max-w-[230px] origin-bottom rotate-[-8deg] rounded-[14px] border border-border bg-white p-3 shadow-sm sm:left-3">
        <div className="h-3.5 w-24 rounded bg-soft" />
        <div className="mt-5 space-y-2">
          <div className="h-2.5 w-full rounded bg-soft" />
          <div className="h-2.5 w-4/5 rounded bg-soft" />
          <div className="h-2.5 w-3/5 rounded bg-soft" />
        </div>
      </div>
      <div className="absolute right-0 top-11 h-[116px] w-[68%] max-w-[266px] rotate-[6deg] rounded-[14px] bg-brand p-3 shadow-lg shadow-emerald-900/10 sm:right-9">
      ...
      <div className="absolute bottom-0 left-1/2 h-[111px] w-[78%] max-w-[304px] -translate-x-1/2 rounded-[14px] border border-border bg-white p-3 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="h-3.5 flex-1 rounded bg-soft" />
          <span className="shrink-0 rounded-full border border-border px-2 py-1 text-[10px] font-bold uppercase text-muted">Copy page</span>
        </div>
...
function AgentFlow() {
  ...
  return (
    <div className="agent-flow mt-16 flex items-center justify-center overflow-hidden sm:mt-20">
      {icons.map((Icon, index) => (
        <div key={index} className="flex items-center">
          <span className={`grid size-12 place-items-center rounded-full sm:size-16 ${index === 3 ? 'bg-white' : 'bg-black/5'}`}>
            <span className={`grid size-10 place-items-center rounded-full sm:size-[53px] ${index === 3 ? 'border-2 border-brand/30 bg-white text-brand' : 'bg-brand text-white'}`}>
              <Icon size={20} />
            </span>
          </span>
          {index < icons.length - 1 && <span className="h-px w-4 bg-border sm:w-8" />}
        </div>
      ))}
    </div>
  );
...
function AssistantMock() {
  return (
    <div className="assistant-mock relative mx-auto mt-12 h-[265px] w-full max-w-[690px] overflow-hidden sm:mt-16">
      ...
      <div className="absolute left-1/2 top-9 z-10 h-[240px] w-[min(270px,calc(100vw-96px))] -translate-x-1/2 rounded-2xl border border-border bg-white p-4 shadow-sm">
